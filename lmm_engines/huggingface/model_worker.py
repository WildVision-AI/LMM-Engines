...
from .constants import ErrorCode, SERVER_ERROR_MSG, ErrorCode
...
class ModelWorker(BaseModelWorker):
    ...
    def generate_stream_gate(self, params):
        ...
        for pop_key in ["model", "echo"]:
            if pop_key in params:
                params.pop(pop_key)
        if self.device == "npu":
            import torch_npu
        ...
        print("generate_stream_gate")
        try:
            if self.seed is not None:
                set_seed(self.seed)
            for output in self.adapter.generate_stream(params):
