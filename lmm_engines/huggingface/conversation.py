...
class SeparatorStyle(IntEnum):
    ...
    METAMATH = auto()
    YUAN2 = auto()
    IDEFICS_SEPSTYLE = auto()
    MINICPMAPI_SEPSTYLE = auto()
...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.ADD_COLON_TWO_IMG:
            seps = [self.sep, self.sep2]
            ret = system_prompt + seps[0]
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ret += role + ": " + ("<image>"+ seps[i % 2] if i == 0 else "") + message + seps[i % 2]
                else:
                    ret += role + ":"
            return ret
            ...
            return ret
        elif self.sep_style == SeparatorStyle.RAW:
            ret = ""
            for _, message in self.messages:
                if message:
                    ...
                    ret += ""
            return ret
        elif self.sep_style == SeparatorStyle.IDEFICS_SEPSTYLE:
            """Convert the conversation to Idefics2 prompt format."""
            if self.system_message == "":
                ret = []
            else:
            ...
            for i, (_, msg) in enumerate(self.messages[self.offset :]):
                if i % 2 == 0:
                    ...
                    if msg is not None:
                        ret.append({"role": "assistant", "content": [{"type": "text", "text": msg}]})
            return ret
        # elif self.sep_style == SeparatorStyle.MINICPMAPI_SEPSTYLE:
        #     """Convert the conversation to minicpm-llama3-v prompt format."""
        #     if self.system_message == "":
        #         ret = []
        #     else:
        #         ret = [{"role": "system", "content": self.system_message}]
        ...
        #     for i, (_, msg) in enumerate(self.messages[self.offset :]):
        #         if i % 2 == 0:
        #             if i == 0: # append image in the first round
        #                 ret.append({"role": "user", "content": [{"type": "text", "pairs": msg}, {"type": "image", "pairs": self.encoded_image}]})
        #             else:
        #                 ret.append({"role": "user", "content": [{"type": "text", "pairs": msg}]})
        #         else:
        #             if msg is not None:
        #                 ret.append({"role": "assistant", "content": [{"type": "text", "pairs": msg}]})
        #     return ret
        else:
    ...
    def set_encoded_image(self, encoded_image):
    ...
    def to_reka_api_messages(self):
        ...
        # TODO: no system message in Reka as in their website
        # if self.system_message == "":
        #     ret = []
        # else:
        #     ret = [{"role": "system", "content": self.system_message}]
        ...
        for i, (_, msg) in enumerate(self.messages[self.offset :]):
            if i == 0:
                ret.append({"type": "human", "text": msg, "media_url": self.media_url, "media_type": self.media_type})
            elif i % 2 == 0:
    ...
    def to_minicpm_messages(self):
        ...
        # ic(self.messages)
        """Convert the conversation to minicpm-llama3-v completion format."""
        if self.system_message == "":
            ret = []
        else:
            ret = [{"role": "system", "content": self.system_message}]
        ...
        for i, (_, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                if i == 0: # append image in the first round
                    ret.append({"role": "user", "contents": [{"type": "text", "pairs": msg}, {"type": "image", "pairs": self.encoded_image}]})
                else:
                    ret.append({"role": "user", "contents": [{"type": "text", "pairs": msg}]})
            else:
                if msg is not None:
                    ret.append({"role": "assistant", "contents": [{"type": "text", "pairs": msg}]})
        return ret

    def to_qwenvlapi_messages(self):
        # from icecream import ic
        # ic(self.messages)
        """Convert the conversation to qwenvlapi completion format."""
        if self.system_message == "":
            ret = []
        else:
            ret = [{"role": "system", "content": self.system_message}]
        ...
        for i, (_, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                # TODO: double check with the team for enabling multi-round
                ret.append({"role": "user", "content": [{"text": msg}, {"image": self.media_url}]})
                # if i == 0: # append image in the first round
                #     ret.append({"role": "user", "content": [{"text": msg}, {"image": self.media_url}]})
                # else:
                #     ret.append({"role": "user", "content": [{"text": msg}]})
            # else:
            #     if msg is not None:
            #         ret.append({"role": "assistant", "content": [{"text": msg}]})
        return ret

    def to_idefics2_messages(self):
        ret = ""
        for i, (_, msg) in enumerate(self.messages[self.offset :]):
            if i < len(self.messages) - 1:
                if i % 2 == 0:
                    # TODO: if no new media_url, also add space after User:
                    ret += f"User:![]({self.media_url}){msg}<end_of_utterance>\n"
                else:
                    if msg is not None:
                        ret += f"Assistant: {msg}<end_of_utterance>\n"
            else:
                if msg is not None:
                    ret += f"Assistant:{msg}"
        return ret
...
# LLaVA v1.6 template
# Ref: https://github.com/haotian-liu/LLaVA/blob/main/llava/conversation.py#L361-L370
register_conv_template(
    Conversation(
        name="llava-v1.6",
        system_message="",
...
# CogVLM template
# Ref: https://github.com/THUDM/CogVLM/blob/main/basic_demo/cli_demo_hf.py
register_conv_template(
    Conversation(
        name="cogvlm",
        system_message="",
...
# MiniCPM-API template
register_conv_template(
    Conversation(
        name="minicpm-llama3-v",
        system_message="",
        # roles=("user", "assistant"),
        # sep_style=SeparatorStyle.MINICPMAPI_SEPSTYLE,
        roles=("", ""),
        sep_style=SeparatorStyle.RAW,
    )
)
...
register_conv_template(
    Conversation(
        ...
        system_message="",
        roles=("USER", "ASSISTANT"),
        sep_style=SeparatorStyle.ADD_COLON_TWO_IMG,
        sep="\n",
    )
)

# DeepSeek VL template
register_conv_template(
    Conversation(
        name="deepseek-vl",
        system_message="",
