...
from ...lita.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from ...lita.utils import load_image, load_video, load_video_frames
...
class BaseDataset(Dataset):
    ...
    def load_image(self, image_path):
        ...
        return load_image(image_path, processor, image_aspect_ratio)
        
    def load_video_frames(self, frame_paths):
        processor = self.data_args.image_processor
        image_aspect_ratio = self.data_args.image_aspect_ratio

        return load_video_frames(frame_paths, processor, None, image_aspect_ratio)
    
    def load_video(self, video_path, num_frames):
        assert self.data_args.image_aspect_ratio is not 'pad'
    ...
    def __getitem__(self, i):
        ...
        i = rng.randint(0, len(self.list_data_dict))  # random index
        ...
        if has_image:
            try:
                image = self.get_visual(sources[0])
            except Exception as e:
                print(e)
                return self.__getitem__(0)  # resample
        ...
        if has_image:
            ...
            # TODO: update zero pad depending on input visual type
